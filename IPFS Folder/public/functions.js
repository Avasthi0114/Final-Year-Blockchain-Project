...
async function uploadFiles(event) {
    ...
    try {
        ...
        const folderCID = result.files[result.files.length - 1].cid; 

        // Update the innerHTML with the correct folder CID
        uploadResultDiv.innerHTML = `
            <h3>Folder Uploaded Successfully!</h3>
            <p><strong>Folder CID:</strong> ${folderCID}</p> <!-- Use extracted folderCID -->
            <h4>Uploaded Files:</h4>
            <ul>${result.files.map(file => `<li>${file.path} (${file.cid})</li>`).join('')}</ul>
        `;
    } catch (error) {
